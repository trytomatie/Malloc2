...
class Skill_Slurp : Skill
{
    ...
    /// </summary>
    public Skill_Slurp(float cooldown, float casttime,bool allowsMovement)
    {
        ...
        this.Name = "Slurp";
        this.Description = "Deals 100% Magic Power to all close enemys. Regnerate Sp for 10% of the damage done.";
        this.SpCost = 0;
        this.Image = ItemIcons.GetSkillIcon(20);
    }
    ...
    IEnumerator ThunderStrike(GameObject source)
    {
        ...
        int damage = (int)(s.MagicPower * scaling);
        ...
        foreach (Statusmanager entity in eligableTargets)
        {
            ...
            fx2.transform.Find("DamageObjectParticles3").gameObject.SetActive(true);
        ...
        while (fxList.Count > 0 && timeout < 4000)
        {
            ...
            foreach (GameObject fx in fxList)
            {
                ...
                {
                    removalList.Add(fx);
                    s.Sp += (int)( fx.GetComponent<DamageObject>().damage * 0.1f);
                }
            }
