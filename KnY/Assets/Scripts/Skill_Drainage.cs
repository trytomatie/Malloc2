...
class Skill_Drainage : Skill
{
    ...
    /// </summary>
    public Skill_Drainage(float cooldown, float casttime,bool allowsMovement)
    {
        ...
        this.Name = "Drainage";
        this.Description = "Deals 75% Dexterity to all close enemys. Heal for 50% of the damage done.";
        this.SpCost = 50;
        this.Image = ItemIcons.GetSkillIcon(19);
    }
    ...
    IEnumerator ThunderStrike(GameObject source)
    {
        ...
        int damage = (int)(s.Dexterity * 0.75f);
        int healAmount = 0;
        ...
        foreach (Statusmanager entity in eligableTargets)
        {
            ...
            fx2.transform.Find("DamageObjectParticles2").gameObject.SetActive(true);
        ...
        while (fxList.Count > 0 && timeout < 4000)
        {
            ...
            foreach (GameObject fx in fxList)
            {
                ...
                { 
                    fx.GetComponent<Rigidbody2D>().velocity = PublicGameResources.CalculateNormalizedDirection(fx.transform.position, source.transform.position) * 2;
                }
                else
                {
                    removalList.Add(fx);
                    s.ApplyHeal(source, fx.GetComponent<DamageObject>().damage);
                }
            }
