...
public class Spline2DComponent : MonoBehaviour {
    ...
    public List<Vector2> GetSplinePositions()
    {
        if (Count == 0)
        {
            return null;
    ...
    public List<Vector2> GetSplinePositions(int i)
    {
        if (Count == 0)
        {
            return null;
        ...
        Vector3 plast = GetPointWorldSpace(0);
        int counter = 0;
        for (float t = sampleWidth * i * stepsPerSegment; t <= 1.0f; t += sampleWidth)
        {
            Vector3 p = InterpolateWorldSpace(t);
            positions.Add(p);
            plast = p;
            if(counter == stepsPerSegment)
            {
                break;
            }
            counter++;
    ...
    private void DrawNormalsGizmo() {
        ...
        float sampleWidth = 0.3f / ((float)Count * stepsPerSegment);
        Gizmos.color = Color.magenta;
        for (float t = 0.0f; t <= 0.3f; t += sampleWidth) {
            Vector3 p = InterpolateWorldSpace(t);
